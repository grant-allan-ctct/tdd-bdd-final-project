...
def step_impl(context):
    ...
    #
    target_url = f"{context.base_url}/products"
    context.response = requests.get(target_url)
    assert(context.response.status_code == HTTP_200_OK)
    for product in context.response.json():
        context.response = requests.delete(f"{target_url}/{product['id']}")
    ...
    for row in context.table:
        ...
        context.response = requests.post(target_url, json=payload)
