...
class TestProductRoutes(TestCase):
    ...
    def test_get_product(self):
        """It should Read information from a product"""
    ...
    def test_get_product_with_invalid_id(self):
        """It should fail to Read any product"""
        test_product = self._create_products()[0]
        invalid_product_id = test_product.id + 1
    ...
    def test_update_product(self):
        """It should Update information about a product"""
        test_product = ProductFactory()
        response = self.client.post(BASE_URL, json=test_product.serialize())
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        # update the product
        ...
        new_product["description"] = test_description
        response = self.client.put(f"{BASE_URL}/{product_id}", json=new_product)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_query_by_name(self):
        """It should Get a list of all the Products with the given name"""
        total_product_count = 5
        products = self._create_products(total_product_count)
        test_name = products[0].name
        tally_expected = len([product for product in products if product.name == test_name])
        query = f"name={quote_plus(test_name)}"
    ...
    def test_query_by_availability(self):
        ...
        total_product_count = 10
        products = self._create_products(total_product_count)
        tally_expected = len([product for product in products if product.available is True])
